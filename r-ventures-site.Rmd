...
## Above function calculates the mean across replicates (because replicate IDs were not included in the group_by argument)
```

After averaging across replicates, I now want sum sequence counts within a specific taxonomic group.

I didn't want to type out the different taxonomic levels into my group_by function, so let's separate those again. THEN summarize to the *Supergroup* level. Additionally, for unassigned ASVs to the *Supergroup* level, the final mutate statement replaces _NA_ with _Eukaryote Unassigned_.
...
```
The difference between ```summarize()``` and ```mutate()```, is that _summarize()_ generates a new data frame that is smaller than your original, while _mutate_ adds a column to your existing data frame.
If the above command prints an error, note that it is likely because some of the separating taxonmy to the species level has left some columns blank.
...
```{r, tidy=FALSE, fig.height=5, fig.width=7}
...
```{r, echo=TRUE, tidy=FALSE, fig.height=5, fig.width=7}
ggplot(asv_supergroup, aes(x = VentID, y = SUM, fill = Supergroup)) +
  geom_bar(stat = "identity", color = "black", position = "fill") +
...
# Supergroup in the order I want
supergroup_in_order <- c("Stramenopiles","Alveolata", "Rhizaria", "Archaeplastida", "Hacrobia", "Amoebozoa", "Apusozoa", "Excavata", "Opisthokonta", "Eukaryote Unassigned")

# Factor, add column - set levels equal to the supergroup character list
asv_supergroup$SUPERGROUP_ORDER <- factor(asv_supergroup$Supergroup, levels = supergroup_in_order)

# List colors
colors_ten <- c("#f768a1","#ef3b2c", "#fd8d3c", "#fff7bc", "#d95f0e", "#7fcdbb", "#238443","#2b8cbe", "#6a51a3","#bcbddc")

# Set Supergroup equal to list of colors
names(colors_ten) <- supergroup_in_order

# View the new column in my data frame
head(asv_supergroup)
```
...
Now that we have our custom color schematic, we can also modify other components of the ggplot using ```theme()```. There are [built in ggplot themes](https://ggplot2.tidyverse.org/reference/ggtheme.html) or there is a long list of cosmetic changes you can make with [theme()](https://ggplot2.tidyverse.org/reference/theme.html).
...
[Source from Stackoverflow](https://stackoverflow.com/questions/7263849/what-do-hjust-and-vjust-do-when-making-a-plot-using-ggplot)
...
As an example, will use the _Star Wars_ dataset through tidyverse again.
...
```
### Facets
Example of pivoting to long format and using ```facet_grid()```.
...
Lets create a section plot called _count_.
...
Use pathwork to stitch them together.
Either side by side:
...
Lets use the _Star Wars_ dataset through tidyverse again.
```{r, echo=TRUE, tidy=FALSE}
library(tidyverse)
# First what is the data I'm looking at on Tatooine
# View(starwars)
...
starwars %>% 
  select(name, height, mass) %>% 
  pivot_longer(cols = c("height", "mass"), names_to = "TRAIT", values_to = "VALUE")
```
...
starwars %>% 
  ...
  ggplot(aes(x = TRAIT, y = name, fill = VALUE)) +
  geom_tile()
```
...
starwars %>% 
  ...
  theme(legend.position = "right", panel.grid.major = element_blank(), 
    ...
    strip.text.y = element_text(hjust = 0, vjust = 0.5, angle = 0), strip.text.x = element_text(hjust = 0, 
        vjust = 0.5, angle = 0)) + labs(x = "Droid trait", y = "")
```
...
To include custom color schematics and sequential breaks see [this page for more information.](https://ggplot2.tidyverse.org/reference/scale_gradient.html)
...
To start again: Grab your own .csv file or download one here. *To download*, right click [here](https://github.com/shu251/r-ventures/blob/main/data-input/asv-gordaridge-18s-wide.csv) and select *Download Linked File As..*. Save this as a *.csv* file in a place you can find again.
...
```{r}
asv_18s_class <- asv_18s_forheat %>% 
  group_by(ASV, Taxon_updated, Site, VentID, SampleType) %>% 
  summarise(AVERAGE = mean(value)) %>% 
  separate(Taxon_updated, c("Kingdom", 
    "Supergroup", "Division", "Class", "Order", "Family", "Genus", "Species"), sep = ";", 
    remove = FALSE) %>% 
  group_by(Supergroup, Division, Class, VentID, SampleType) %>% 
  summarise(SUM = sum(AVERAGE)) %>%
  # Replace NAs in the Supergroup column
  mutate(Supergroup = replace_na(Supergroup, "Eukaryote Unassigned"))
```
...
asv_18s_class %>% 
  ...
  mutate(SAMPLE_TOTAL = sum(SUM),
         RelAbun = SUM/SAMPLE_TOTAL) %>% 
  # Change x and y parameters, and fill with Relative Abundance
  ggplot(aes(x = VentID, y = Class, fill = RelAbun)) +
    geom_tile() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, color = "black", face = "bold", 
                                   size = 12, hjust = 1, vjust = 1),
...
asv_18s_class %>% 
  ...
  theme(legend.position = "right", panel.grid.major = element_blank(), 
    ...
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, color = "black", 
        size = 8), axis.text.y = element_text(color = "black", size = 8), strip.background = element_blank(), 
    strip.text.y = element_text(hjust = 0, vjust = 0.5, angle = 0), strip.text.x = element_text(hjust = 0, 
        vjust = 0.5, angle = 0)) + labs(x = "Samples", y = "Class") +
  scale_fill_gradient(
    low = "#ffeda0",
    high = "#bd0026",
    space = "Lab",
...
To explore these plots in more detail, see these examples:
* [Hu et al. 2021 code](https://shu251.github.io/protist-gordaridge-2021/#71_Re-curate_taxonomic_levels_(higher_resolution))

* [Hu et al. in review](https://shu251.github.io/microeuk-amplicon-survey/#22_Heat_map_at_class_level)


## Other resources for plotting data

* [Advanced ggplot in R](https://microbiaki.github.io/workshop_t2/) | Software Carpentries course taught at WHOI
